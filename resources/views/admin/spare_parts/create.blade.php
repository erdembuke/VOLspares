<x-app-layout>
    <div class="max-w-xl mx-auto p-8 bg-white shadow rounded-lg">
        <h1 class="text-2xl font-bold mb-6 text-center">Add New Spare Part</h1>
        <form method="POST" action="{{ route('spare-parts.store') }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="model" class="w-full rounded border-gray-300" required>
            ...
            <div>
                ...
                <select name="year" class="w-full rounded border-gray-300" required>
                    <option value="" disabled selected>Select</option>
                    @for($y = 1990; $y <= 2025; $y++)
            ...
            <div>
                ...
                <select name="part_brand" class="w-full rounded border-gray-300" required>
                    <option value="" disabled selected>Select</option>
                    @foreach(['VOLVO (Genuine)', 'Bosch', 'Valeo', 'NGK', 'LONGSERNG', 'Monroe'] as $brand)
                        <option value="{{ $brand }}" {{ old('part_brand') == $brand ? 'selected' : '' }}>{{ $brand }}</option>
                    @endforeach
                </select>
                @error('part_brand')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div class="flex justify-between">
                ...
                <button type="submit"
                        class="px-6 py-2 rounded bg-blue-600 text-white hover:bg-blue-700 font-bold">Save</button>
            </div>
        </form>
    </div>
</x-app-layout>
